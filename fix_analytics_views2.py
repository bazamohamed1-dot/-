...
# Indentation in the file was missing 8 spaces so let's adjust it
# Use simple replace to avoid regex issues with replace string
old_block_to_replace = """filtered_class_map = {}
        for lvl, clist in class_map.items():
            valid_cls = []
            for c in clist:
                from .analytics_utils import unformat_class_name
...
content = content.replace(old_block_to_replace, new_filter_block.strip())
